...
public class ShadowBarbEnchantment extends RangedEnchantment {
    public ShadowBarbEnchantment(Properties properties) {
        super(properties);
    }
    //public ShadowBarbEnchantment(Rarity rarity, EnchantmentTarget enchantmentTarget, EquipmentSlot[] equipmentSlots) {
    //    super(rarity, enchantmentTarget, equipmentSlots);
    //    if (McdwCommon.CONFIG.mcdwEnchantmentsConfig.ENCHANTMENT_CONFIG.get(EnchantmentsID.SHADOW_BARB).mcdw$getIsEnabled()) {
    ...
    public boolean isAcceptableItem(ItemStack stack) {
        return stack.getItem().equals(McdwCrossbowItemRegistry.CROSSBOW_VEILED_CROSSBOW);
    }
